            ...
            <div class="sidebar__bg">
                ...
                >
                    ...
                    <img
                        class="icon"
                        v-else-if="
                            item.disabled === false && item.done === true
                        "
...
const playStartAudio = (audioPath) => {
    startedAudio.value.src = new URL(
...
const switchTask = (id, openId, time, img, audio_task, startAudioV) => {
    ...
    if (task.disabled === false) {
        ...
        startAudio.value = startAudioV; // перенести
        show.value = false; // отображение кнопки - перенести
        taskAudio.value = audio_task; // озвучка задания - перенести
        endTime.value = false; // конец задания - отказано
        emit('sendImg', img);
            ...
            switch (taskId.value) {
                case 1:
                    ...
                    audio.value.onended = () => {
                        ...
                        emit('sendPreAudio', isPlaying.value);
                        isPlaying.value = true;
                        playAudio('Other/10.общее.mp3', id); // "Эта речь похожа на музыку" 
                        emit('sendPreAudio', isPlaying.value);
                        postAudio('Task1/11.1_.mp3');
                    }
